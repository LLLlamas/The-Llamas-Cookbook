...
Last refreshed: 2026-05-17 (session 18 — UserDefaults caching for plan + friends, stale-while-revalidate, refresh button removed, ImportFromPhotoView swipe-down blocked).
...
- **`ImportFromPhotoView` always sets `.interactiveDismissDisabled(true)`** — swipe-down is blocked in all states (not just during OCR). The rubber-band-and-snap-back gesture reads as "minimize"; blocking it forces the user to tap Cancel explicitly. The Cancel button in `.cancellationAction` is the only exit path. Do not narrow this back to `ocrInProgress` — it should remain `true` unconditionally.
