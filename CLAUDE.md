...
**Keyboard toolbars inside a paging `TabView` attach per-field, not at the NavigationStack.** `placement: .keyboard` declared on the carousel's outer `.toolbar` doesn't reliably propagate down into a TextField nested inside a `TabView(.page)` — the bar surfaces on some focuses and vanishes on others. The fix used in `PhotoCarouselView`'s caption editor: declare `ToolbarItemGroup(placement: .keyboard)` directly on each TextField via `.toolbar { ... }`. Each per-page CaptionRow declares its own; SwiftUI surfaces only the focused field's toolbar so the user always sees one Done above the keyboard. Pair this with a lifted-up `@FocusState<Int?>` on the carousel so the Done button can resign focus on whichever page is editing (`captionFocus.wrappedValue = nil`).
