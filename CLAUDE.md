...
- **[Recipe-Sharing.md](./Recipe-Sharing.md)** — **PR 1 + PR 2 landed, PR 3 next**. Adds app-to-app recipe sharing via a `.llamarecipe` file (UTType + JSON envelope including base64 photos) and a `llamascookbook://recipe/v1/<base64url>` deep-link variant for photoless recipes. **Done:** schema (`LCRecipeShareV1`), encode/decode/materialize (`Lib/RecipeShare.swift`), `OwnerProfile` Observable, provenance fields on `Recipe`, UTType + `LSSupportsOpeningDocumentsInPlace` Plist additions (PR 1); Detail share menu with `ShareSheet` UIActivityViewController wrapper + first-share name prompt + 350ms post-alert delay (PR 2). **Not yet done:** receiver-side `onOpenURL` recipe branch, `RecipeImportPreviewView`, provenance line render in Detail (PR 3); separate Share Extension target so the app appears in OTHER apps' share sheets (PR 4 — see [Share-Extension-Plan.md](./Share-Extension-Plan.md)).
- **[Share-Extension-Plan.md](./Share-Extension-Plan.md)** — **planned, not started**. Sibling to Recipe-Sharing.md. Adds a separate `LlamasCookbookShareExtension` app-extension target (`com.llamascookbook.app.shareext`) so Llamas Cookbook appears as a destination in iOS share sheets across all apps (Safari, Files, Mail, Reddit, etc.). Activation rule accepts `public.url` and `com.llamascookbook.recipe` UTIs. Architecture is **transparent passthrough** — extension reads the NSItemProvider, hands off to the main app via deep link (URLs encoded inline as `llamascookbook://share-url/<base64>`, `.llamarecipe` files written to an App Group shared container at `group.com.llamascookbook.app/share-inbox/<uuid>.llamarecipe` and handed off via `llamascookbook://share-incoming/<uuid>`). No SwiftData in the extension, no duplicated parsers. Apple Developer Portal setup is on the user (~5–15 min); see Share-Extension-Plan.md §8 for the step-by-step.
...
Active queue: recipe sharing between users (PR 1 + PR 2 shipped — schema, UTType, Detail share menu; PR 3 = inbound import flow per [Recipe-Sharing.md](./Recipe-Sharing.md), PR 4 = Share Extension per [Share-Extension-Plan.md](./Share-Extension-Plan.md)), then per-cook `TimerLiveActivityRegistry`, then the aesthetic / typography pass.
...
| Recipe Detail | `Views/Detail/RecipeDetailView.swift` | Sections, gallery + step-photo thumbnails, share menu (file / link / text), sourdough chip, "Add to Cook Mode" green button when a cook is already active. |
| Favorite toggle | `RecipeDetailView` | Heart toolbar, syncs `updatedAt`. |
| Export (text) | `Lib/RecipeExport.swift` + share menu | Plain-text out — Notes, Messages, Mail, AirDrop. Third option in the share menu, alongside `.llamarecipe` file and URL-deep-link forms. |
| Recipe sharing (file / link) | `Lib/RecipeShare.swift` + `Views/Components/ShareSheet.swift` | App-to-app share via `.llamarecipe` UTType (file form, includes photos) or `llamascookbook://recipe/v1/<base64url>` deep link (URL form, photo-less recipes only — gated on `hasAnyPhotos`). State-machine driven: first-share name prompt → `UIActivityViewController` via `ShareSheet`. Receiver-side import not yet wired (PR 3). |
...
1. **Recipe sharing between users**, per [Recipe-Sharing.md](./Recipe-Sharing.md) + [Share-Extension-Plan.md](./Share-Extension-Plan.md). Four-PR sequence: PR 1 schema/UTType/provenance fields (✅ shipped) → PR 2 outbound Detail share menu + first-share prompt (✅ shipped) → **PR 3 inbound** (`onOpenURL` recipe branch + Import Preview + provenance display, handles AirDrop + chat-link) → **PR 4 Share Extension** (Llamas Cookbook appears in other apps' share sheets — Safari, Files, Reddit, etc.). Cloud transport designed in but not implemented.
