...
- Accent-cascade sequence (`startAccentTransition`) is strictly ordered: header + categories pre-roll (t=0 / 0.04), then `recipeList` stage activates at t=0.10 and bumps `recipeCardCascadeToken` — each `RecipeCardView` reads its `index` and self-schedules its own glow with `recipeCardGlowStagger` (0.05s) delay so the list ripples top → bottom one card at a time. Plus button fires at t=0.55, bottom tab bar at t=0.72, state clears at t=0.95. `LibraryView` MUST pass `index:` to `RecipeCardView` (via `filtered.enumerated()`) or the stagger collapses. Per-card glow lives in local `@State` driven by `.onChange(of: recipeCardCascadeToken)` — do NOT route it back through `isAccentGlowActive(.recipeList)` (that flag still drives `LetterIndex` and the recipe-list color advance and must stay a single shared boolean)
