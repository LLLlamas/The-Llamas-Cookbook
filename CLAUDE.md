...
- **Strict confidence gate (IG only)**: `.full` requires `title + ingredients + steps` ALL populated. Looser drafts route to `.insufficientForImport(enrichment:hint:)` which `ImportFromTextLinkView` surfaces as a "Write it down myself" capsule below the warning banner. Tap clears `editor.hasUnsavedChanges` and calls `editor.startNew(seed: enrichment)` — `EditorCoordinator` swaps the sheet content in-place to `RecipeEditorView` with hero photo + source URL + `@creator` summary pre-filled. TikTok / Pinterest / generic-blog imports keep their looser `.partial` behavior; this gate is IG-only.
- **Hero photo: extract first frame from mp4, NOT `og:image`**. IG bakes its white play triangle into the `og:image` pixels for any video post and we can't CSS it off. `InstagramExtractor.extractFirstFrame(from:)` uses `AVAssetImageGenerator` at t=0.1s (not 0 — IG sometimes serves a black first frame) with 0.5s `requestedTimeToleranceAfter` to tolerate keyframe spacing. JPEG-encoded at quality 0.85 via `ImageIO`. Returns nil (never throws) on any failure. Photo/carousel posts (no `og:video`) fall back to `og:image` — they have no play-icon overlay.
...
- **Strict IG confidence gate** — `RecipeURLImporter.fetchInstagram` now requires title + ingredients + steps ALL populated to return `.full`. Looser AI drafts route to new `Outcome.insufficientForImport(enrichment:hint:)` which `ImportFromTextLinkView` renders as a "Write it down myself" capsule button that hands the enrichment to `EditorCoordinator.startNew(seed:)` — sheet content swaps in-place to a near-empty `RecipeEditorView` with hero photo + source URL + `@creator` summary pre-filled. Avoids the prior fail mode of dumping users back into the import sheet with a half-filled paste box.
- **Clean hero photo from mp4 first-frame** — IG bakes a white play triangle into `og:image` for any video post. New `InstagramExtractor.extractFirstFrame(from:)` pulls a clean frame at t=0.1s via `AVAssetImageGenerator`; photo/carousel posts (no mp4) keep `og:image` (no overlay to worry about).
- **Parallel mp4 + thumbnail + AI parse** — mp4 download, thumbnail download, AI parse, and first-frame extraction are now structured-concurrency parallel via `async let` + `Task.detached`. Stage 1 runs AI parse alongside hero-photo resolution; stage 2 runs frame-extract alongside transcription on the shared mp4 file. Shaves 1–3 s off perceived wall-clock vs. the prior serial chain.
