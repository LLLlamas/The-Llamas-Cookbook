...
- `View.liftedCard()` (`Components/View+Lifted.swift`) — static drop-shadow (radius 6, y 3, 0.13 opacity) for non-interactive card surfaces. Applied outside `.clipShape()` so the shadow isn't clipped.
- `LiftedButtonStyle` / `.buttonStyle(.lifted)` (`Components/View+Lifted.swift`) — interactive press-down style: shadow flattens + scaleEffect 0.96 on press, 0.12s easeOut. Use instead of manual shadow + scaleEffect on any tappable pill or button.
...
- **Photo-import flow** — `ImportFromPhotoView.runImport` prepares pages in two formats in parallel (`.aiVision` pixel-capped JPEG ≤1568px long edge AND ≤1.2MP; `.ocr` 2560px). Cascade: (1) on-device OCR + `RecipeImporter.parse` → `localPhotoParseConfident` gate (≥3 ingredients with qty/unit, ≥2 steps ≤220 chars, explicit section label in text) — free if confident; (2) **streaming Sonnet vision** via Cloudflare proxy (`RecipeAIParser.parseImagesStreaming`) — the processing overlay stays visible during Anthropic TTFB (5–10 s for vision) showing the llama + `"Asking the llama…"` status text; `StreamingRecipeState.onFirstContent` fires when the title is extracted and presents `PhotoImportPreviewView` as a `fullScreenCover` at that moment — completely covering `ImportFromPhotoView` so the user sees only the blank preview; content ticks in immediately as events land; (3) OCR text reused for `parseBestOf(preferHighQuality:true)` fallback if the stream produced no usable draft. Cache hits (non-streaming) never fire `onFirstContent` — the success branch pops the preview directly when `preview == nil`. The legacy "What are we cookin'?" title input and "Ready / Review Recipe" state were removed — the processing overlay is now just a llama + status text. Banner "Edit as text" is the final fallback. **Anthropic vision rejects HEIC** — `aiVision` forces JPEG via `forcesJPEGOutput`. Vision timeout 60s; same 429/529 backoff as text. OCR text is computed once and reused by the AI fallback — never recognized twice. Per-import branch + timing is logged via `os_signpost` + a single `Logger.info` summary line under subsystem `com.llamascookbook.app`, category `photoImport` (never logs recipe content).
