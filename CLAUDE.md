...
- **`os.Logger` diagnostics** — `InstagramExtractor` + `RecipeURLImporter.Instagram` log subsystems trace videoURL / thumbnailURL / carousel-slide presence and which fallback tier supplied the final hero photo. Filter Console.app on `com.llamascookbook.app` subsystem when triaging a "still shows play overlay" report.
