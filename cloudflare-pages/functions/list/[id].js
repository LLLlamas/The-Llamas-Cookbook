...
import { parseGroceryRecord, groupByAisle, measureText } from '../../lib/grocery.js';
...
export function renderListHTML({ title, items }) {
  ...
  const sectionsHTML = sections.map((section) => {
    const header = showAisles
      ? `<h2 class="aisle">${escapeHTML(section.aisle)}</h2>`
      : '';
    const rows = section.items.map(renderItem).join('');
    return `${header}<ul class="items">${rows}</ul>`;
  }).join('');
...
<head>
  ...
  <meta name="description" content="A shared grocery list from Llamas Cookbook.">
  <meta property="og:title" content="${safeTitle}">
  <meta property="og:description" content="A shared grocery list — tap to shop it.">
  ...
  <style>
    :root {
      ...
      --done: #8aa68a; --warn: #b5613c; --shadow: rgba(58,45,30,.08);
    }
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --done: #9bbd9b; --warn: #e0875f; --shadow: rgba(0,0,0,.5);
      }
    }
    * { box-sizing: border-box; }
    ...
    .progress.done { color: var(--done); background: rgba(138,166,138,.14); }
    .aisle {
...
<body>
  <main>
    <header>
      <img class="logo" src="/llama-icon.png" alt="">
      <h1>${safeTitle}</h1>
...
function renderItem(item) {
  const safeName = escapeHTML(sanitizedOr(item.name, 'Item'));
  ...
  return `<li class="item${item.checked ? ' checked' : ''}">
    <span class="check${item.checked ? ' on' : ''}">${item.checked ? '✓' : ''}</span>
    <span class="body"><span class="name">${measureHTML}${safeName}</span>${noteHTML}</span>
  </li>`;
}
