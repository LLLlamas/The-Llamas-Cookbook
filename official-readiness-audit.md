...
### 🟣 Llama Pro Tier UX — Fixed (Session 17)

**15. PaywallView llama icon reflects subscription tier**
- Was: `LlamaLogo(size: 96, ...)` — always showed base llama
- Now: `LlamaLogoOrCrown(size: 96, accent: AppColor.accent)` — shows base / crown / crown+sunglasses based on current plan

**16. PaywallView plan picker is tier-aware**
- Free users: both Yearly ("Best value" badge) and Monthly cards shown
- Monthly users: only Yearly card shown (they're upgrading, not re-buying)
- Yearly users: paywall is never surfaced (no chip points to it)
- Rule: never show a plan card the user already holds

**17. RecipeDetailView principal toolbar icon resized**
- Was: `size: 72`, `.padding(.top, 22).padding(.bottom, 22)` — read as oversized
- Now: `size: 52`, `.padding(.top, 32).padding(.bottom, 36)` — proportional bar height ~120pt

**18. Terracotta brand color enforced on all tier surfaces**
- Files: `ProfileView.swift`, `ImportFromPhotoView.swift`, `PaywallView.swift`
- All tier pills, upgrade chips, and paywall accent → `AppColor.accent` (`#C97C5D`)
- Was using `appearance.accentColor` (user-customizable) — made brand surfaces drift with user preference

**19. Upgrade chip logic is plan-aware on all surfaces**
- `.none` → Monthly + Yearly chips; `.monthly` → "Switch to Yearly" only; `.yearly` → `EmptyView()`
- Applied consistently in `ProfileView.planPillRow`, `ImportFromPhotoView.upgradeChips`, `PaywallView.planPicker`

**20. ImportFromPhotoView llama and quota pill updated**
- Hero llama and blocked-card llama → `LlamaLogoOrCrown` (tier-aware)
- Quota pill foreground/background → `AppColor.accent` (terracotta, not user accent)

**21. 🔴 Critical fix: monthly→yearly upgrade didn't update UI**
- File: `ios-native/Sources/Views/Profile/PaywallView.swift`
- Was: `.onChange(of: proStore.isPro)` — never fires on monthly→yearly (isPro was already true)
- Now: `.onChange(of: proStore.plan)` — fires on every tier transition including monthly→yearly
- Combined with: `Task { await quotaService.refresh(force: true) }` runs before dismiss

**22. 🔴 Critical fix: StoreKit entitlements not synced on PaywallView open**
- File: `ios-native/Sources/Views/Profile/PaywallView.swift`
- Was: `.task` only called `loadProduct()` — stale plan from prior session never detected
- Now: `await proStore.checkCurrentEntitlements()` runs first in `.task`; if plan changes, `onChange(of: proStore.plan)` auto-dismisses before picker is interacted with. Catches subscriptions purchased in prior sessions where the old dismiss bug prevented cleanup.

*Generated by Claude Code — architecture engineering skill. Last updated session 17 (2026-05-17): Llama Pro tier UX fixes, PaywallView crown icon, plan picker tier-awareness, terracotta brand color enforcement, critical monthly→yearly dismiss fix, StoreKit entitlement sync on paywall open.*
