...
struct PhotoCarouselView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            // per-field TextField — was unreliable here: the bar would
            // miss the first focus and only appear after some other
            // modal cycle (e.g. opening + closing the reorder sheet)
            // primed SwiftUI's toolbar discovery. `safeAreaInset` is
            // deterministic — keyboard avoidance lifts whatever sits
            // here above the keyboard, and the conditional render is
            // driven by our own focus state so the bar shows the moment
            // any caption focuses. Sits above the keyboard's suggestions
            // strip; visually equivalent to the missing accessory.
            .safeAreaInset(edge: .bottom, spacing: 0) {
                captionKeyboardAccessory
            }
        }
    ...
    /// Custom keyboard accessory bar — shown only while a caption
    /// TextField is the focused first responder. Lives in the bottom
    /// safe-area inset so iOS's keyboard avoidance pushes it directly
    /// above the keyboard (above the suggestions strip), giving the
    /// user the same visual position they'd get from a native
    /// `placement: .keyboard` toolbar — minus the SwiftUI flakiness
    /// that's specific to nested TabView pages inside a sheet.
    /// Tapping Done resigns whichever caption is focused
    /// (`captionFocusedPage = nil`), which fires the CaptionRow's
    /// commit funnel and dismisses the keyboard.
    @ViewBuilder
    private var captionKeyboardAccessory: some View {
        if captionFocusedPage != nil {
            HStack(spacing: 0) {
                Spacer()
                Button {
                    Haptics.selection()
...
private struct CaptionRow: View {
    ...
    private var editor: some View {
        ...
        // carousel-level `captionKeyboardAccessory` bar (a
        // `safeAreaInset(edge: .bottom)` view that iOS's keyboard
        // avoidance lifts above the keyboard) rather than a
        // SwiftUI `placement: .keyboard` toolbar — the latter is
        // unreliable in this nested TabView-in-sheet context.
        .submitLabel(.return)
