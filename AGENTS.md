...
Last refreshed: 2026-08-23 (first hardware feedback on grocery push: on-device push diagnostics + per-group registration logging added, `.denied` authorization surfaced, live-`!` poll ceiling cut 30 s → 10 s, inbound merges made monotonic; 226 Swift + 115 JS green)

Previous refresh: 2026-08-18 (grocery lists re-verified against the tree: full suite green — 216 Swift + 115 JS — file inventory and all four known gaps confirmed unchanged since 2026-08-09; no doc drift found)

Earlier refresh: 2026-08-09 (shared grocery lists verified end-to-end: CloudKit schema confirmed deployed in Production, both grocery pushes fixed, check-off sync latency cut, shared-list toast + deep link added, `Sources/Views/Lists/` split into feature-sized files)
...
- **Reads** — on a push, `scheduleCoalescedRefresh` refreshes the list ON SCREEN first (one `record(for:)`, tracked implicitly via `refreshSharedList`) before the two-query full reconcile. Detail view also polls every 3 s, easing to a **10 s ceiling** (`GroceryListDetailView.visiblePollInterval`) — the owner's subscription is content-available only and iOS throttles silent pushes, so the poll is often what actually delivers. It was 30 s until 2026-08-23; that ceiling was itself the "you have to back out and re-enter to see the `!`" bug, because re-entry restarts the `.task` and resets the counter. A FAILED fetch backs off separately and harder (`failureBackoff`, 5 s → 60 s) so a rate-limited container isn't polled at on-screen cadence. Both are locked by `GrocerySyncGuardTests`.
- **INVARIANT — the sync loop owns the head of its own `.task`.** `runVisibleSharedListSync` used to sit behind `await autoTriage()` (an on-device model round trip) and its own `await requestVisibleNotificationAuthorizationIfNeeded()` (a MODAL system alert). Both blocked the first fetch indefinitely. They now run as sibling `.task(id: liveSyncKey)` modifiers. Never put an unbounded await ahead of the first `refreshSharedList`.
- **INVARIANT — inbound merges are monotonic per record.** `record(for:)` is strongly consistent; the `CKQuery`s behind `refresh()` are only EVENTUALLY consistent, and a push fires both. Without an ordering rule the stale query result overwrites the fresh read and visibly reverts a check-off. `GroceryListStore.shouldApply` drops any snapshot strictly older than one applied within `staleSnapshotGuardWindow` (10 s). The window EXPIRES on purpose: `updatedAt` is stamped by the writing client, so skewed clocks can go backwards and an unbounded guard would drop the slower phone's writes forever. A brand-new recipient mirror bypasses the guard — it has no content yet.
...
- **INVARIANT — permission must be asked before someone can be a recipient.** Visible pushes need `UNUserNotificationCenter` authorization; iOS silently drops the alert half when `.notDetermined`. Requested from `RootView.task` (right after `registerIfNeeded`), `ListsView.task`, `ShareGroceryListSheet`, and `GroceryListDetailView`. The Lists-tab one is load-bearing: a pure recipient has never shared or opened a shared list, so without it the push announcing the list is the one being suppressed. The `RootView` one covers the case where the Lists gate (`UserProfileMirror.cachedRecordID() != nil`) is still cold the first time the tab opens — `.task` won't re-run, so that user sits at `.notDetermined` forever. **Never `.provisional`**: it auto-grants without prompting and demotes every banner to Notification-Centre-only, which looks identical to the bug.
- **INVARIANT — `.denied` must stay visible.** iOS never re-prompts after a refusal, and **cook timers use AlarmKit's SEPARATE authorization** — so alarms keep firing and the app looks healthy while every CloudKit push is silently dropped. That combination cost the whole 2026-08 two-device run. `requestVisibleNotificationAuthorizationIfNeeded` returns the resulting status, and `ListsView` shows a banner into `UIApplication.openSettingsURLString` when it is `.denied`.
- **Diagnostics** — `CloudKitSubscriptions.diagnostics()` reads back authorization, alert setting, APNs registration + last failure, the gate key, and which of the seven subscription IDs actually exist (`publicDB.allSubscriptions()`). Surfaced as the **PUSH NOTIFICATIONS** row in `ProfileView` settings. This is the only way to inspect a SECOND tester's account — CloudKit Console only shows the account you can sign in as. `expectedSubscriptionIDs(for:)` is the single source of truth shared with `unregisterAll`.
...
- **Registration is per-group, not one chain.** `registerIfNeeded` runs friendship / recipeImport / grocery as three independent `do`/`catch`es and logs the failing group + `CKError` via `os.Logger`. It was one chained `do`, so a throw in the friendship group (saves 1-2 of 7) silently starved the four grocery subscriptions behind it — indistinguishable from a network blip, forever, with no log line. The gate is only set when every group lands.
- **KNOWN GAP — the personalized first-share push can only fire once per list.** `upsertShare` creates the `GroceryListShare` row exactly once and updates thereafter, but `grocery-list-shared-<me>` is `.firesOnRecordCreation` only. Adding a recipient later, re-sharing, or sharing a list that already existed all take the update path, so the recipient gets the generic "list changed" body instead of the personalized banner. Fixing it needs a per-recipient invite record type (schema deploy + `.v6` gate bump).
...
| `GrocerySyncGuardTests.swift` | on-screen poll cadence + its 10 s ceiling, failure backoff, and the monotonic-merge rule (`shouldApply`) incl. its deliberate expiry |
...
**Blocking v1.2.0 — one real two-device run against Production.** Everything below is code-complete and green in CI/simulator (226 Swift + 115 JS tests pass as of 2026-08-23), but none of it can be proven from a build machine. **Start with the PUSH NOTIFICATIONS row in Profile settings on BOTH devices** — it reports authorization, APNs registration, the gate key and 7-of-7 subscriptions, and discriminates every failure mode below in one reading:
